...
void UAbilityAsync_WaitGameplayTagQuery::Activate()
{
	...
	if (ASC != nullptr)
	{
		...
		for (const FGameplayTag& Tag : QueryTags)
		{
			...
			{
				UpdateTargetTags(Tag, ASC->GetTagCount(Tag), Tag, EOnGameplayEffectTagCountOperation::INTERNALCHECK);
...
void UAbilityAsync_WaitGameplayTagQuery::UpdateTargetTags(const FGameplayTag Tag, int32 NewCount, const FGameplayTag TriggerTag, EOnGameplayEffectTagCountOperation TagOperation)
{
